...
@router.get("/", response_model=List[schemas.HearingInDB])
async def get_all_hearings(
    skip: int = Query(0, ge=0),
    limit: int = Query(100, ge=1, le=500),
    db: AsyncSession = Depends(get_db),
    current_user: models.User = Depends(get_current_active_user)
):
    """Listar todas as audiências dos processos do escritório."""
    ...
    query = select(models.Hearing).filter(
...
@router.post("/", response_model=schemas.HearingInDB, status_code=status.HTTP_201_CREATED)
async def create_hearing(
    ...
    case_query = select(models.Case).filter(
        models.Case.id == hearing.case_id,
    ...
    db_hearing = models.Hearing(**hearing.model_dump())
    db.add(db_hearing)
    await db.commit()
    await db.refresh(db_hearing)
    return db_hearing
